...
## 2. Explore cumulative days below a threshold
...
## 4. Explore degree days

Previous studies have found degree days and growing degree days (gdd) to be good predictors of phenology [@guo2019; @cayton2015]

Degree day = (daily max + daily min) / 2

Growing degree day (gdd) = sum of degree days since a given date (I chose August 1)
...
Drop day has oscillated with precipitation and the day of the mean drop-day-GDD.

## 5. Model

**LM** (julian_drop \~ gdd) had a significant slope and an adjusted R\^2 of 0.54

**GLM** (julian_drop \~ gdd + gdd_day + cum_precip) had significant slopes for all predictors with 80% deviance explained

Got stuck on **Binomial GLM and GAM** response plots (see S1 Fig)
...
Data for weather from this fall from [@noaanationalestuarineresearchreservesystem2019]
...
**The model may be better if:**

-   It included historic drop day as a predictor (a sort of naive component)

-   A binary model may also resolve the confidence interval issues because the training dataset would be much larger (S1 Fig)

![](images/clipboard-3397317430.png)

**S1 Fig:** QQ plot for binomial GLM.

![](images/clipboard-414602589.jpeg)

**S2 Fig:** Texts with a friend.
